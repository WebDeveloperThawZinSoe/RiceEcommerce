            ...
            <div class="row">
                                ...
                                <div class="dz-content-start">
                                    ...
                                    <div class="product-tag">
                                        <span class="badge bg-purple mb-2">Sale |
                                            ...
                                            {{$detail_product->discount_amount}} Ks OFF
                            ...
                            <div class="meta-content m-b20 d-flex align-items-end">
                                ...
                                @if($detail_product->product_type == 1)
                                    @if($detail_product->variants->count() > 0)
                                        @php
                                            $firstVariant = $detail_product->variants->first();
                                        @endphp
                                        @if(isset($firstVariant->expire_date))
                                            <div class="variant-expire-date mt-2">
                                                <label class="form-label">Expire Date:</label>
                                                <span>{{ $firstVariant->expire_date }}</span>
                                            </div>
                                        @endif
                                    @endif
                                @endif
                                @if($detail_product->product_type == 2)
                                 @if($detail_product->variants->count() > 0)
                                        ...
                                        @if(isset($firstVariant->expire_date))
                                            <div class="variant-expire-date mt-2">
                                                <label class="form-label">Expire Date:</label>
                                                <span id="product-expire">{{ $firstVariant->expire_date }}</span>
                                            </div>
                                        @endif
                                    @endif
                                @endif
                            ...
                            @if($detail_product->variants->count() > 0)
                                <div class="product-variants">
                                    ...
                                    <div class="variant-buttons d-flex flex-wrap gap-2">
                                        @php
                                            // Sort by attribute_name in ascending order
                                            $groupedVariants = $detail_product->variants->sortBy('attribute_value')->groupBy('attribute_name');
                                        @endphp
                                        @foreach($groupedVariants as $attributeName => $variants)
                                            <div class="variant-group" style="width:100% !important;">
                                                ...
                                                @foreach($variants as $variant)
                                                    @if($variant->stock != 0 && $variant->status == 1)
                                                        @php
                                                            ...
                                                            $v_expire = $variant->expire_date ?? "N/A";
                ...
                document.querySelectorAll('.variant-option').forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        document.getElementById('product-price').innerText = variantPrice + ' Ks';
                        ...
                        // Update the expire date
                         const variantExpireDate = button.getAttribute('data-expire');
                        document.getElementById('product-expire').innerText = variantExpireDate ;
